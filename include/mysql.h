...
#if defined(_WIN64)
#define ma_socket unsigned long long
#elif defined(_WIN32)
#define ma_socket unsigned int
#else
typedef int ma_socket;
#endif
#endif
#endif
#include "mysql_com.h"
...
#define INTERNAL_NUM_FIELD(f) (((f)->type <= MYSQL_TYPE_INT24 && ((f)->type != MYSQL_TYPE_TIMESTAMP || (f)->length == 14 || (f)->length == 8)) || (f)->type == MYSQL_TYPE_YEAR)
  ...
  typedef unsigned long ma_ulonglong;
#elif defined (_WIN32)
  typedef unsigned __int64 ma_ulonglong;
#else
  typedef unsigned long long ma_ulonglong;
#endif
...
#define MYSQL_COUNT_ERROR (~(ma_ulonglong) 0)
...
struct st_mysql_options {
    ...
    char *ma_cnf_file,*ma_cnf_group, *charset_dir, *charset_name;
    ...
    ma_bool use_ssl;				/* if to use SSL or not */
    ma_bool compress,named_pipe;
    ma_bool unused_1, unused_2, unused_3, unused_4;
    enum mysql_option methods_to_use;
    char *bind_address;
    ma_bool secure_auth;
    ma_bool report_data_truncation; 
  ...
  typedef struct st_mysql {
    ...
    ma_ulonglong insert_id;		/* id if insert on table with NEXTNR */
    ma_ulonglong extra_info;		/* Used by mysqlshow */
    ...
    ma_bool free_me;		/* If free in mysql_close */
    ma_bool reconnect;		/* set to 1 if automatic reconnect */
    ...
    void          *thd;
    ma_bool       *unbuffered_fetch_owner;
    char          *info_buffer;
    void          *extension;
} MYSQL;
...
typedef struct st_mysql_res {
  ...
  ma_bool	eof;			/* Used my mysql_fetch_row */
  ma_bool       is_ps;
} MYSQL_RES;
...
void ma_set_error(MYSQL *mysql, unsigned int error_nr, 
...
int		STDCALL mysql_send_query(MYSQL *mysql, const char *q,
					 size_t length);
ma_bool	STDCALL mysql_read_query_result(MYSQL *mysql);
...
ma_socket STDCALL mysql_get_socket(MYSQL *mysql);
...
int STDCALL mysql_commit_start(ma_bool *ret, MYSQL * mysql);
int STDCALL mysql_commit_cont(ma_bool *ret, MYSQL * mysql, int status);
int STDCALL mysql_rollback_start(ma_bool *ret, MYSQL * mysql);
int STDCALL mysql_rollback_cont(ma_bool *ret, MYSQL * mysql, int status);
int STDCALL mysql_autocommit_start(ma_bool *ret, MYSQL * mysql,
                                   ma_bool auto_mode);
int STDCALL mysql_autocommit_cont(ma_bool *ret, MYSQL * mysql, int status);
...
int STDCALL mysql_stmt_close_start(ma_bool *ret, MYSQL_STMT *stmt);
int STDCALL mysql_stmt_close_cont(ma_bool *ret, MYSQL_STMT * stmt, int status);
...
int STDCALL mysql_set_character_set_cont(int *ret, MYSQL *mysql,
                                                  int status);
int STDCALL mysql_change_user_start(ma_bool *ret, MYSQL *mysql,
                                                const char *user,
                                                const char *passwd,
                                                const char *db);
int STDCALL mysql_change_user_cont(ma_bool *ret, MYSQL *mysql,
                                               int status);
...
int             STDCALL mysql_fetch_row_cont(MYSQL_ROW *ret, MYSQL_RES *result,
                                             int status);
int             STDCALL mysql_read_query_result_start(ma_bool *ret,
                                                      MYSQL *mysql);
int             STDCALL mysql_read_query_result_cont(ma_bool *ret,
...
int STDCALL mysql_stmt_reset_start(ma_bool *ret, MYSQL_STMT * stmt);
int STDCALL mysql_stmt_reset_cont(ma_bool *ret, MYSQL_STMT *stmt, int status);
int STDCALL mysql_stmt_free_result_start(ma_bool *ret, MYSQL_STMT *stmt);
int STDCALL mysql_stmt_free_result_cont(ma_bool *ret, MYSQL_STMT *stmt,
                                        int status);
int STDCALL mysql_stmt_send_long_data_start(ma_bool *ret, MYSQL_STMT *stmt,
                                            unsigned int param_number,
                                            const char *data,
                                            size_t len);
int STDCALL mysql_stmt_send_long_data_cont(ma_bool *ret, MYSQL_STMT *stmt,
                                           int status);
...
struct st_mariadb_api {
  ...
  ma_bool (STDCALL *mysql_change_user)(MYSQL *mysql, const char *user, const char *passwd, const char *db);
  ...
  void (STDCALL *mysql_free_result)(MYSQL_RES *result);
  void (STDCALL *mysql_data_seek)(MYSQL_RES *result, ma_ulonglong offset);
  MYSQL_ROW_OFFSET (STDCALL *mysql_row_seek)(MYSQL_RES *result, MYSQL_ROW_OFFSET);
...
struct st_mysql_methods {
  ...
  /* prepared statements */
  ma_bool (*db_supported_buffer_type)(enum enum_field_types type);
  ma_bool (*db_read_prepare_response)(MYSQL_STMT *stmt);
  int (*db_read_stmt_result)(MYSQL *mysql);
  ma_bool (*db_stmt_get_result_metadata)(MYSQL_STMT *stmt);
  ma_bool (*db_stmt_get_param_metadata)(MYSQL_STMT *stmt);
