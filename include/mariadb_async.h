...
extern ssize_t ma_send_async(MARIADB_PVIO *pvio,
                             ...
                             int timeout);
extern my_bool ma_io_wait_async(struct mysql_async_context *b,
                                enum enum_pvio_io_event event, int timeout);
#ifdef HAVE_TLS
#include <ma_tls.h>
extern int ma_ssl_read_async(struct mysql_async_context *b, MARIADB_TLS *tls,
                             void *buf, int size);
extern int ma_ssl_write_async(struct mysql_async_context *b, MARIADB_TLS *tls,
